...
parser.add_argument('--test_folder', default='./test_images/',
...
if __name__ == '__main__':
    ...
    if (1):
        ...
        # load data
        image_list = [
            "https://persist.signzy.tech/api/files/45016609/download/fhQatpDVwr6je7xyGNEBk6bhBvREo4sAbEPTZ05Lm9zBe8sd52.png"]
    ...
    img = imgproc.loadImage(
        "https://persist.signzy.tech/api/files/45016609/download/fhQatpDVwr6je7xyGNEBk6bhBvREo4sAbEPTZ05Lm9zBe8sd52.png")
    lines = []
    with open(r".\result\res_fhQatpDVwr6je7xyGNEBk6bhBvREo4sAbEPTZ05Lm9zBe8sd52.txt", "r") as file_in:

        for count, line in enumerate(file_in):
            vals = line.split(",")
            vals = [int(i) for i in vals]
            xmin = min(vals[0], vals[2], vals[4], vals[6])
            xmax = max(vals[0], vals[2], vals[4], vals[6])
            ymin = min(vals[1], vals[3], vals[5], vals[7])
            ymax = max(vals[1], vals[3], vals[5], vals[7])
            # try:
            cropped_image = img[ymin:ymax, xmin:xmax]
            cv2.imwrite("crop_images/crop_" +
                        str(count)+".jpg", cropped_image)
            image = Image.open("crop_images/crop_" +
                               str(count)+".jpg").convert("RGB")
            ...
            results = ["crop_"+str(count)+".jpg",
                       generated_text, xmin, xmax, ymin, ymax]
            lines.append(results)
            # except Exception as e:
            #     print(e)

    with open(r'foriegn.txt', 'w') as fp:
        for item in lines:
